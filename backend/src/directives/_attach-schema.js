...
import { FORBIDDEN, NOT_ALLOWED } from "../constants";
...
export const attachDirectives = schema => {
  forEachField(schema, field => {
    ...
    directives.forEach(async directive => {
      ...
      const expectedScopes = directive.arguments.reduce(
        (arr, arg) => [...arr, ...arg.value.values.map(v => v.value)],
        []
      );

      if (!expectedScopes || (expectedScopes && expectedScopes.length < 1))
        throw Error(
          "A custom directive constraint not recognized. Please check the correctness of its name"
        );
      ...
      if (resolver) {
        ...
        field.resolve = async function resolve() {
          ...
          let promise = originalResolve
            ? originalResolve.call(field, ...arguments)
            : source;
